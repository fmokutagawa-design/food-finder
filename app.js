...
function visualHtml(r){
  ...
  return `<a class="food-visual ${r.photoUrl?'has-photo':''}" href="${destination}" target="_blank" rel="noopener" ${r.photoUrl?`style="background-image:linear-gradient(180deg,transparent 35%,rgba(9,19,14,.75)),url('${r.photoUrl}')"`:''}><span>${r.photoUrl?(r.photoLabel||'公式写真'):`${r.genres[0]}の写真を見る`}</span><b>写真 ↗</b></a>`;
}
...
function openDetail(id){
  ...
  $("#detailContent").innerHTML=`<p class="eyebrow">RESTAURANT FILE</p><div class="detail-title"><div><p>${r.areaLabel}・徒歩${r.walk}分</p><h2>${r.name}</h2></div><span class="status ${st.state}"><i></i>${st.label}</span></div>${ratingsHtml(r)}<p class="detail-summary">${r.summary}</p><div class="detail-cta">${r.menuUrl?`<a href="${r.menuUrl}" target="_blank" rel="noopener">メニューを見る <span>↗</span></a>`:''}${r.photosUrl?`<a href="${r.photosUrl}" target="_blank" rel="noopener">料理写真を見る <span>↗</span></a>`:''}</div><div class="detail-facts"><div><span>指定日時の判定</span><b>${st.detail}</b></div><div><span>本日の通常営業</span><b>${hoursToday(r,date)}</b></div><div><span>予算目安</span><b>¥${r.budget.toLocaleString()}前後</b></div><div><span>予約</span><b>${r.reserve?"予約先あり":"未確認"}</b></div><div><span>持ち帰り</span><b>${r.takeout?"確認あり":"未確認"}</b></div><div><span>配達</span><b>${r.delivery?"確認あり":"未確認"}</b></div></div><div class="address"><span>所在地</span><p>${r.address}</p></div><h3 class="media-title">媒体を横断して確認</h3><div class="media-links">${r.media.map(m=>`<a href="${queryUrl(m,r.name)}" target="_blank" rel="noopener"><span>${m}</span><small>検索して確認 ↗</small></a>`).join("")}</div><div class="source-box"><span>主要情報源</span><a href="${r.official}" target="_blank" rel="noopener">${r.sourceLabel} ↗</a><small>最終確認 ${r.checked}　｜　点数は同日確認時点。臨時休業・貸切・満席は反映されない場合があります。</small></div>`;
