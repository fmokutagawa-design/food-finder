...
function activeLimitedMenus(date){return (window.LIMITED_MENUS||[]).filter(m=>date>=new Date(m.start)&&(!m.end||date<=new Date(m.end)))}
function renderLimited(date){const items=activeLimitedMenus(date);$("#limitedSection").hidden=!items.length;$("#limitedMenus").innerHTML=items.map(m=>{const shops=m.restaurantIds.map(id=>RESTAURANTS.find(r=>r.id===id)).filter(Boolean);return `<article class="limited-card"><div><span>${m.kind}</span><small>${m.brand}</small></div><h3>${m.name}</h3><p>${m.summary}</p><b>${m.price}</b><small class="availability">${m.availability}</small><div>${shops.map(r=>`<button data-limited-shop="${r.id}">${r.name}のカルテ →</button>`).join('')}<a href="${m.url}" target="_blank" rel="noopener">公式発表 ↗</a></div><em>${m.checked}確認</em></article>`}).join('');$$('[data-limited-shop]').forEach(b=>b.onclick=()=>openDetail(b.dataset.limitedShop))}
function fitScore(r,status){return status.score+(r.confidence==="high"?8:4)+(r.reserve?2:0)-r.walk/3}
function search(scroll=false){const date=getDate(), area=$("#area").value,budget=+$("#budget").value,openOnly=$("#openOnly").checked,reservable=$("#reservable").checked,chefOnly=$("#chefOnly").checked,gentleOnly=$("#gentleOnly").checked,delivery=$("#delivery").checked;currentResults=RESTAURANTS.map(r=>({...r,_status:statusAt(r,date)})).filter(r=>(selectedGenre==="すべて"||r.genres.includes(selectedGenre))&&(area==="all"||(area==="station"?['station','west','east'].includes(r.area)&&r.walk<=5:r.area===area))&&r.budget<=budget&&(!openOnly||["open","caution"].includes(r._status.state))&&(!reservable||r.reserve)&&(!chefOnly||r.chefUrl)&&(!gentleOnly||r.gentle)&&(!delivery||r.takeout||r.delivery));renderLimited(date);sortAndRender();const dateText=new Intl.DateTimeFormat("ja-JP",{month:"numeric",day:"numeric",weekday:"short",hour:"2-digit",minute:"2-digit"}).format(date);$("#resultNote").innerHTML=`<span>${dateText}</span> の通常営業時間から判定。全${RESTAURANTS.length}店から表示しています。臨時変更・満席は公式サイトで最終確認してください。`;if(scroll)$("#resultsSection").scrollIntoView({behavior:"smooth",block:"start"})}
...
function evidenceHtml(r){
  const items=[];
  if(r.chefUrl)items.push(`<a class="evidence chef" href="${r.chefUrl}" target="_blank" rel="noopener"><b>料理人紹介あり</b><small>${r.chefName}・${r.chefSpecialty} ↗</small></a>`);
  else if(r.hitosaraUrl)items.push(`<a class="evidence listed" href="${r.hitosaraUrl}" target="_blank" rel="noopener"><b>ヒトサラ</b><small>店舗ページ ↗</small></a>`);
...
$("#searchButton").onclick=()=>search(true);$("#sort").onchange=sortAndRender;["dateTime","area","budget","openOnly","reservable","chefOnly","gentleOnly","delivery"].forEach(id=>$("#"+id).onchange=()=>search());
$("#chefOnly").onchange=()=>{if($("#chefOnly").checked)$("#openOnly").checked=false;search()};
